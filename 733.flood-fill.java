...
class Solution {
    public int[][] floodFill(int[][] image, int sr, int sc, int color) {
        ...
        if(sr<0 || sr>=m ||sc<0 || sc>=n || image[sr][sc]==color)return image;
        
        solve(image,sr,sc,color,image[sr][sc]);
        return image;
    }
    
    public void solve(int[][] img,int r,int c,int ncol,int col){
        if(r<0 || c<0 || r>=img.length || c>=img[0].length || img[r][c]!=col)return ;
        img[r][c]=ncol;
        solve(img,r+1,c,ncol,col);
        solve(img,r,c+1,ncol,col);
        solve(img,r-1,c,ncol,col);
        solve(img,r,c-1,ncol,col);
    }
}
